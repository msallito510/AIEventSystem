...
router.get("/view/:id/", function (req, res, next) {
  ...
  Event.find({});
  Event.findOne({ _id: id }).populate('members').then(data => {
...
router.get("/likes", function (req, res, next) {
  ...
  Event.find({ likes: userid }).populate('likes').then(data => {
...
router.post("/like/:id/", function (req, res, next) {
  ...
  Event.findById({ _id: id }, { likes: iduser }).populate('likes').then((data) => {
    ...
    if (data.likes.length == 0) {
      ...
      Event.findByIdAndUpdate({ _id: id }, { $pull: { likes: iduser } }).then((data) => {
        console.log(data)
        res.redirect('/')
      })
    }
